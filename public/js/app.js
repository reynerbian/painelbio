...
const leftIcon = document.querySelector('.left-icon');
        ...
        window.startUploadSite = async function(arroba) {
            ...
            setProgress(15, 'Preparando réplica estática...');
            addScraperLog('[Upload] Preparando réplica do site...', 'info');
            await new Promise(r => safeSetTimeout(r, 400));
        ...
        window.openSiteInfoModal = async function(arroba) {
            ...
            const saveContactInfo = () => {
                ...
                if (itemIdx !== -1) {
                    ...
                    if (badge) {
                        badge.style.opacity = '1';
                        safeSetTimeout(() => { badge.style.opacity = '0'; }, 1500);
                    }
                }
            };
            ...
            <script>
                (function() {
                    ...
                    if (isSlide) {
                        ...
                        safeSetTimeout(runSlideCycle, 500);
                    } else {
                        if (texts.length > 1) {
            ...
            if (isTopbannerActive) {
                ...
                if (phoneScreen && texts.length > 0) {
                    ...
                    if (window.phoneTbConfigKey !== currentConfigKey) {
                        ...
                        const safeSetTimeout = (fn, ms) => {
                            const id = safeSetTimeout(fn, ms);
                            window.phoneTbTimers.push(id);
                            return id;
                        };
                        const safeSetInterval = (fn, ms) => {
                            const id = safeSetInterval(fn, ms);
                            window.phoneTbIntervals.push(id);
                            return id;
                        };
