...
loadClassicModel();
        ...
        // Função global utilitária para extrair os dados atuais do formulário do editor
        window.getFormData = function() {
            const arrobaInput = document.getElementById('input-arroba');
            let cleanArroba = arrobaInput ? arrobaInput.value.trim().toLowerCase() : '';
            if (cleanArroba && !cleanArroba.startsWith('@')) {
        ...
        inspectorContentContainer.addEventListener('submit', async (e) => {
            ...
            const updatedData = window.getFormData();
            const cleanArroba = updatedData.arroba;
            if (!cleanArroba) {
