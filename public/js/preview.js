...
function updatePreviewFromForm() {
            ...
            if (isTopbannerActive) {
                ...
                if (phoneScreen && texts.length > 0) {
                    ...
                    if (window.phoneTbConfigKey !== currentConfigKey) {
                        ...
                        // Reset de estilos iniciais do banner e do elemento de texto
                        phoneTopBanner.style.transform = 'none';
                        phoneTopBanner.style.opacity = '1';
                        ...
                        if (txtEl) {
                            ...
                            txtEl.style.transition = 'none';
                        }

                        // Configura transições e estilos específicos de cada efeito no início
                        if (effect === 'slide') {
                            phoneTopBanner.style.transition = 'transform 0.45s cubic-bezier(0.16, 1, 0.3, 1), opacity 0.3s';
                        } else if (effect === 'fade') {
                            if (txtEl) txtEl.style.transition = 'opacity 0.3s ease-in-out';
                        } else if (effect === 'bounce') {
                            if (txtEl) txtEl.style.transition = 'transform 0.5s cubic-bezier(0.175, 0.885, 0.32, 1.275), opacity 0.3s';
                        } else if (effect === 'flip') {
                            if (txtEl) txtEl.style.transition = 'transform 0.4s ease-in, opacity 0.3s';
                            phoneTopBanner.style.perspective = '500px';
                        ...
                        function runLiveEffectCycle() {
                            ...
                            if (effect === 'marquee') {
                                // O efeito Marquee é contínuo e tem fluxo próprio
                            ...
                            window.phoneTbInterval = safeSetTimeout(() => {
                                ...
                                // 2. Troca de texto e efeito de ENTRADA
                                // O tempo do timeout deve ser suficiente para completar a animação de saída
                                const transitionDuration = (effect === 'slide') ? 450 : 400;
                                ...
                                safeSetTimeout(() => {
                                    ...
                                    // Continua o ciclo recursivamente
                                    runLiveEffectCycle();
                                }, transitionDuration);
            ...
            if (isEmojiRainActive) {
                ...
                const erSway = document.getElementById('input-addon-er-sway')?.checked || false;
                const erOpacity = document.getElementById('select-addon-er-opacity')?.value || 'normal';
                const erAsSvg = document.getElementById('input-addon-er-as-svg')?.checked || false;
                const erSvgColor = document.getElementById('input-addon-er-svg-color')?.value || '#7c3aed';
                ...
                const configKey = `${erEmoji}_${erCount}_${erSpeed}_${erCoverage}_${erRotate}_${erSway}_${erOpacity}_${erAsSvg}_${erSvgColor}`;
                if (window.phoneErConfigKey !== configKey) {
                    ...
                    styleEl.textContent = `
                        @keyframes pb-emojifall    { 0%{top:-80px;opacity:0} 10%{opacity:${opacityVal}} 90%{opacity:${opacityVal}} 100%{top:100%;opacity:0} }
                        @keyframes pb-emojifall-cw { 0%{top:-80px;transform:rotate(0deg);opacity:0} 10%{opacity:${opacityVal}} 90%{opacity:${opacityVal}} 100%{top:100%;transform:rotate(540deg);opacity:0} }
                        @keyframes pb-emojifall-ccw{ 0%{top:-80px;transform:rotate(0deg);opacity:0} 10%{opacity:${opacityVal}} 90%{opacity:${opacityVal}} 100%{top:100%;transform:rotate(-540deg);opacity:0} }
                        @keyframes pb-emojisway { 0% { margin-left: -12px; } 100% { margin-left: 12px; } }
                    `;
                    ...
                    const emojiSvgPaths = {
                        ...
                        'alien': '<path d="M12 2a9 9 0 0 0-9 9c0 3.3 1.5 6.3 4 8v3h10v-3c2.5-1.7 4-4.7 4-8a9 9 0 0 0-9-9zM8 12.5a1.5 2.5 0 1 0 3 0 1.5 2.5 0 1 0-3 0zm5 0a1.5 2.5 0 1 0 3 0 1.5 2.5 0 1 0-3 0z"/>',
                    ...
                    for (let i = 0; i < count; i++) {
                        ...
                        const size = (1.0 + Math.random() * 1.4).toFixed(2); // Variabilidade maior de 1.0rem a 2.4rem
                        const left = (Math.random() * 90).toFixed(1);
                        
                        // Efeito 3D Depth of Field (Opção 1)
                        const szVal = parseFloat(size);
                        let blurVal = 1.5;
                        let speedMultiplier = 1.0;
                        if (szVal > 1.9) {
                            blurVal = 0.3; // Fica bem nítido pois está perto
                            speedMultiplier = 0.75; // Fica mais rápido pela perspectiva (mais perto)
                        } else if (szVal < 1.4) {
                            blurVal = 3.5; // Fica bem borrado no fundo
                            speedMultiplier = 1.35; // Fica mais lento
                        }
                        
                        const duration = (baseDuration * speedMultiplier * (0.8 + Math.random() * 0.4)).toFixed(2);
                        ...
                        // Renderiza o emoji como SVG ou Caractere Normal
                        if (erAsSvg && emojiSvgPaths[emoji]) {
                            span.innerHTML = `<svg viewBox="0 0 24 24" width="100%" height="100%" style="fill:none;stroke:${erSvgColor};stroke-width:2.2px;stroke-linecap:round;stroke-linejoin:round;display:block;">${emojiSvgPaths[emoji]}</svg>`;
                        } else {
                            span.textContent = emoji;
                        }
                        
                        span.style.cssText = `position:absolute;top:-80px;opacity:0;left:${left}%;font-size:${size}rem;width:1.2em;height:1.2em;display:inline-block;filter:blur(${blurVal}px);pointer-events:none;animation:${animName} ${duration}s linear ${delay}s infinite backwards${swayAnim};`;
            ...
            // =========================================================================
            // ADD-ON 10: JANELA INTERATIVA / POP-UP DE ENTRADA (PREVIEW EM TEMPO REAL)
            // =========================================================================
            ...
            if (isSplashActive && phoneScreen) {
                const splashMode = document.getElementById('select-addon-splash-mode')?.value || 'roulette';
                ...
                if (window.phoneSplashConfigKey !== currentConfigKey) {
                    ...
                    // Injetar estilos CSS locais para animações
                    ...
                    if (splashMode === 'roulette') {
                        ...
                        contentArea.innerHTML = `
                            ...
                            <div id="phone-splash-roulette-status" style="font-size: 0.7rem; color: #64748b; font-weight: 600; text-transform: uppercase;">
                                Sorteando Desconto...
                            </div>
                        `;
            ...
            if (isGlitchActive) {
                ...
                const animDur = glitchSpeed === 'fast'
                    ? (glitchDuration === 'long' ? '0.8s' : '0.4s')
                    : (glitchDuration === 'long' ? '2.5s' : '1.5s');
...
async function loadTemplatePreview(templateId, dataToFill = null) {
            ...
            if (payload && (payload.arroba || payload.name)) {
                ...
                const fieldsToRestore = {
                    ...
                    'select-addon-er-opacity': backup.addonEmojiRainOpacity || 'normal',
                    'input-addon-er-svg-color': backup.addonEmojiRainSvgColor || '#7c3aed',
                    ...
                    'select-addon-splash-mode': backup.addonSplashpromoMode || 'roulette',
                    ...
                    'input-addon-splash-overlay-opacity': backup.addonSplashpromoOverlayOpacity || 0.85,
                    'input-addon-splash-btn-text': backup.addonSplashpromoBtnText || 'Garantir Desconto & Entrar 🚀'
                };
                ...
                const erSwayEl = document.getElementById('input-addon-er-sway');
                if (erSwayEl && backup.addonEmojiRainSway !== undefined) {
                    erSwayEl.checked = Boolean(backup.addonEmojiRainSway);
                }
                const erAsSvgEl = document.getElementById('input-addon-er-as-svg');
                if (erAsSvgEl && backup.addonEmojiRainAsSvg !== undefined) {
                    erAsSvgEl.checked = Boolean(backup.addonEmojiRainAsSvg);
                    const erSvgColorContainer = document.getElementById('container-addon-er-svg-color');
                    if (erSvgColorContainer) {
                        erSvgColorContainer.style.display = erAsSvgEl.checked ? 'flex' : 'none';
                    }
                }
...
function initAuroraEngine(canvas, config) {
    ...
    function loop() {
        localTimeSec += 0.016 * speedMult;
        const timeMs = localTimeSec * 1000;
